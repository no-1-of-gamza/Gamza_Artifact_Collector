...
class RecycleBin:
    ...
    # 아티팩트 수집 -> main 기능
    def collect(self, result_path, drive_list):
        # 수집 환경 세팅
        self.check_os()
        ...
        for drive in self.drive_list:
            print(drive + ". . .")
            dir_path = os.path.join(result_path, drive)
            ...
            for root, dirs, files in os.walk(drive+self.artifact_path[1]):
                for dir in dirs:
                    dir_path = os.path.join(result_path, drive, dir)
                    self.create_dir(dir_path)
                for file in files:
                    # dump
                    src = os.path.join(root, file)
                    dst = os.path.join(result_path, drive, root.split("\\")[2])
                    #print("src: "+src+"\n dst: "+dst)
...
if __name__ == "__main__":
    ...
    artifact.collect(result_path, artifact.drive_list)
